{{-- resources\views\pages\shared\request-ticket.blade.php --}}
@extends('layouts.layout')

@section('content')
    <div class="flex items-center justify-center min-h-[80vh]">
        <div class="bg-white shadow-md border border-gray-200 rounded-lg p-8 w-full max-w-lg text-center space-y-6">

            @if(session('success'))
                <div class="alert alert-soft alert-success border border-green-300 text-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        <!--Boxicons v3.0.8 https://boxicons.com | License  https://docs.boxicons.com/free-->
                        <path
                            d="M19 9.09V6c0-.55-.45-1-1-1h-3.09L12.7 2.79a.996.996 0 0 0-1.41 0L9.08 5H5.99c-.55 0-1 .45-1 1v3.09L2.78 11.3a.996.996 0 0 0 0 1.41l2.21 2.21v3.09c0 .55.45 1 1 1h3.09l2.21 2.21c.2.2.45.29.71.29s.51-.1.71-.29l2.21-2.21h3.09c.55 0 1-.45 1-1v-3.09l2.21-2.21a.996.996 0 0 0 0-1.41l-2.21-2.21Zm-1.71 4.71a1 1 0 0 0-.29.71v2.5h-2.5c-.27 0-.52.11-.71.29L12 19.09l-1.79-1.79a1 1 0 0 0-.71-.29H7v-2.5c0-.27-.11-.52-.29-.71l-1.79-1.79 1.79-1.79A1 1 0 0 0 7 9.51v-2.5h2.5c.27 0 .52-.11.71-.29L12 4.93l1.79 1.79c.19.19.44.29.71.29H17v2.5c0 .27.11.52.29.71l1.79 1.79z">
                        </path>
                        <path d="m11 12.59-1.29-1.3-1.42 1.42 2.71 2.7 4.71-4.7-1.42-1.42z"></path>
                    </svg>
                    {{ session('success') }}
                </div>
            @endif

            @if(session('error'))
                <div class="alert alert-soft alert-error border border-red-300 text-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        <!--Boxicons v3.0.8 https://boxicons.com | License  https://docs.boxicons.com/free-->
                        <path d="M11 7h2v6h-2zm0 8h2v2h-2z"></path>
                        <path
                            d="M12 22c5.51 0 10-4.49 10-10S17.51 2 12 2 2 6.49 2 12s4.49 10 10 10m0-18c4.41 0 8 3.59 8 8s-3.59 8-8 8-8-3.59-8-8 3.59-8 8-8">
                        </path>
                    </svg>
                    {{ session('error') }}
                </div>
            @endif

            <h1 class="text-2xl font-semibold text-gray-800">
                REQUEST TRIP TICKET
            </h1>

            <p class="text-gray-600 text-sm">
                Click the button below to request a trip ticket.
                Admins will review your request and create the ticket with full details.
            </p>

            <form method="POST" action="{{ route('ticket.request.store') }}">
                @csrf
                <button type="submit" class="btn btn-primary w-full">
                    Request Trip Ticket
                </button>
            </form>

        </div>
    </div>
@endsection